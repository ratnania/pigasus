...
! Author: root
!
! Created on February 5, 2012, 5:12 PM
!
...
contains
    ...
    !----------------------------------------------------------------------------------------------
    !> this routine assemblies terms : int ( v . grad phi_b ) phi_bprime
    subroutine build_advection_Local(ao_ASS, ao_FEM, ai_grids_id, ai_id, ai_elt, ai_color, apr_Operator_elt)
        implicit none
        ...
        real(wp), dimension (ao_ASS % oi_maxndof, ao_ASS % oi_maxnpts, mi_MAXDIM) :: lpr_gradB
        real(wp), dimension (ao_ASS % oi_maxndof,ao_ASS % oi_maxnpts) :: lpr_Bprime
        ...
        real(wp), dimension(ao_FEM % oi_maxcoloraddto, ao_ASS % oi_maxnpts, NPARAM_MATRIX) :: lpr_m
...
#ifdef _DEBUG
        call printlog("build_advection_Local : Begin", ai_dtllevel = mi_dtllevel_base + 1)
        ...
        DO li_op_ref = 1, li_noperators
            ...
            lpr_m (li_op_ref, 1:li_npts, 1:li_nparam) = TRANSPOSE( &
            ...
            do li_bprime = 1, li_nenprime
                ...
                DO li_op_ref = 1, li_noperators
                    DO li_d = 1, li_dim
                        ...
                        + lpr_gradB (1, 1:li_npts, li_d)  &
                        * lpr_m(li_op_ref, 1:li_npts, li_d)   
                    END DO
                    lpr_contribution (li_op_ref,1:li_npts) = lpr_contribution (li_op_ref,1:li_npts) &
        ...
        PRINT *, "build_advection_Local : End"

#ifdef _DEBUG
        call printlog("build_advection_Local : End", ai_dtllevel = mi_dtllevel_base + 1)
#endif
    end subroutine build_advection_Local
