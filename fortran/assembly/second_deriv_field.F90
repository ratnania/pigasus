!     
! File:   second_deriv_field.F90
! Author: root
!
! Created on October 10, 2012, 9:53 AM
!

module second_deriv_field_module
    use used_precision
    use tracelog_module
    use grids_def
    use geometries_def
    ...
    public :: build_second_deriv_field_Local
...
contains
    ...
    subroutine build_second_deriv_field_Local(ao_ASS, ao_FEM, ai_grids_id, ai_id, ai_elt, ai_field, apr_fieldh)
        implicit none
        ...
        INTEGER, PARAMETER :: MAXNPARAM = 3
        real(wp), dimension(MAXNPARAM,ao_ASS % oi_maxnpts) :: lpr_HessianB
...
#ifdef _DEBUG
        call printlog("build_second_deriv_field_Local : Begin", ai_dtllevel = mi_dtllevel_base + 1)
#endif
!print *, 'ai_elt=', ai_elt
        ...
        do li_b = 1, lp_con % opi_nen(ai_id)
            ...
            DO li_pt=1, li_npts
            lpr_HessianB (1:li_nparam,li_pt) = lp_pBasis % opr_HessianB (1:li_nparam, li_b, li_pt)
            END DO            

            apr_fieldh(ai_field,1,1:li_nparam, 1:li_npts) = apr_fieldh(ai_field,1,1:li_nparam, 1:li_npts) &
            + ao_FEM % opo_F(li_field_id) % opr_c(li_A) &
            * lpr_HessianB (1:li_nparam,1:li_npts)

        end do

#ifdef _DEBUG
        call printlog("build_second_deriv_field_Local : End", ai_dtllevel = mi_dtllevel_base + 1)
#endif
    end subroutine build_second_deriv_field_Local


end module second_deriv_field_module
